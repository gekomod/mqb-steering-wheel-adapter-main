...
class AccHandle {
    ...
    const uint8_t ACC_SPEED_PLUS_PIN = 5;
    const uint8_t ACC_SPEED_MINUS_PIN = 6;
    const uint8_t ACC_RESUME_PIN = 17;
    const uint8_t ACC_AST_PIN = 18;
    ...
    const uint8_t ACC_DIST_PLUS_PIN = 16;
    const uint8_t ACC_DIST_MINUS_PIN = 14;
    const uint8_t ACC_SET_PIN = 15;
public:
    bool accOn = false;
    ...
    void loop(uint8_t pressedAccButton) {
        ...
        uint8_t ACC_AST = (pressedAccButton == lib_bus_mqb::ACC_AST) ? HIGH : LOW;
        ...
        digitalWrite(ACC_AST_PIN, ACC_AST);
        // if (DEBUG_ACC == 2) {
        //     DebugLog("\nACC handle: ");
